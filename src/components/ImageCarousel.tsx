...
export function ImageCarousel({
  ...
  return <div className="relative w-full">
    ...
    <div className="relative aspect-[3/4] overflow-hidden bg-[#F5F0EB] rounded-sm">
      {images.map((image, index) => <img key={index} src={image} alt={`${productName} - ${imageLabels[index] || `View ${index + 1}`}`} className={`absolute inset-0 w-full h-full object-contain transition-opacity duration-300 ease-in-out ${index === currentIndex ? 'opacity-100' : 'opacity-0'}`} />)}
